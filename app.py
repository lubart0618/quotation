...
def build_quote_pdf(data: dict) -> bytes:
    ...
    payment_method = str(data.get("paymentTerms", "") or "-").replace("\n", "<br/>")
        ...
        [
            [
                Paragraph(
                    ...
                    f"電話：&nbsp;&nbsp;{data.get('contactPhone', '')}<br/>"
                    f"地址：&nbsp;&nbsp;{data.get('companyAddress', '')}",
                    body_style,
                ),
                Paragraph(
                    f"報價單號：&nbsp;&nbsp;{data.get('quoteNumber', '')}<br/>"
                    f"日期：&nbsp;&nbsp;{data.get('quoteDate', '')}<br/>"
                    f"報價有效期：&nbsp;&nbsp;{data.get('validUntil', '')}<br/>"
                    f"稅率：&nbsp;&nbsp;{tax_rate:.0f}%",
                    body_style,
                ),
            ]
    ...
    for item in items:
        ...
        table_rows.append(
            [
                ...
                Paragraph(str(qty), small_style),
                Paragraph(money(price), small_style),
                Paragraph(money(amount), small_style),
            ]
        )
    ...
    summary_table = Table(
        [["小計", money(subtotal)], ["稅額", money(tax)], ["總金額", money(total)]],
        ...
        [
            [
                Paragraph(
                    ...
                    f"統編：{QUOTE_PROVIDER['company_id']}<br/>"
                    f"地址：{QUOTE_PROVIDER['address']}<br/>"
                    f"電話：{QUOTE_PROVIDER['phone']}<br/>"
                    f"專案執行人：{executor or '________________'}",
                    small_style,
                ),
